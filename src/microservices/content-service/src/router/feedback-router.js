...
router.get("/", async (req, res) => {
  ...
  try {
    ...
    const users = await feedbackService.getAllFeedback(
      search,
...
router.get("/:id", async (req, res) => {
  try {
    const user = await feedbackService.getAllFeedback(req.params.id);
    if (user) {
      res.json(user);
    } else {
...
router.post("/", async (req, res) => {
  try {
    const newUser = await feedbackService.createFeedback(req.body);
